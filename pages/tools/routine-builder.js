...
// --- BASE DE DATOS COMPLETA DE RUTINAS (RESTAURADA) ---
...
const OptionCard = ({ value, label, icon, selectedValue, onClick }) => {
  ...
  return (<button onClick={handleClick} className={`p-4 border rounded-lg text-center transition-all duration-200 flex flex-col items-center justify-center h-full min-h-[110px] ${isSelected ? 'bg-pink-500 text-white border-pink-500 shadow-lg scale-105' : 'bg-white hover:bg-pink-50 hover:shadow-md'}`}><span className="text-3xl">{icon}</span><span className="block mt-2 font-semibold text-sm">{label}</span></button>);
};
