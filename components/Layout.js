...
// --- NUEVO COMPONENTE DE BÚSQUEDA (CON LÓGICA COMPLETA) ---
const SearchModal = ({ isOpen, onClose }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [results, setResults] = useState([]);
  const [allPosts, setAllPosts] = useState([]);

  // Obtenemos la lista de todos los posts UNA SOLA VEZ cuando el modal se abre por primera vez
  useEffect(() => {
  ...
  // Filtramos los resultados cada vez que el usuario escribe
  useEffect(() => {
  ...
  // Limpiamos todo cuando se cierra el modal
  useEffect(() => {
    if (!isOpen) {
      setTimeout(() => {
        setSearchTerm('');
      ...
      <div className="flex flex-col min-h-screen bg-gray-50 text-gray-800">
        ...
        <div className={`fixed top-0 left-0 h-full w-full max-w-xs bg-white shadow-lg z-30 transform transition-transform duration-300 ease-in-out md:hidden ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
            <div className="pt-24 px-6 space-y-4">
              ...
              {/* ... (el resto de tus enlaces del menú móvil) ... */}
            </div>
        </div>
